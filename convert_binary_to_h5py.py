...
"""
    ...
    return save_info
"""
def binary_reader(file_directory, header_struct, body_struct,                     constants,              reading_param,const_param):
"""
"""
first 4 bytes are a one-float header
first 4 bytes are a one-float header
    ...
    with open(file_name,"rb") as halofile:
        ...
        while binary_info:
            ...
            for index in range(len(reading_param)):
                list[index].append(constants[const_param[index]]*                                               halo_info[reading_param[index]])
            binary_info = halofile.read(halo_struct.size)
    return list
...
def convert_array_to_h5py(file_adress,directory_list,datasets_list,data):
    ...
    for name_index in range(len(directories)):
        ...
        for data_index in range(len(data_sets[name_index])):
            create_file[directories[name_index]].create_dataset(data_sets[                           name_index][data_index],data = data_save[                                    reading_index])
            reading_index = reading_index + 1


#calling functions defined above
info = binary_reader("/cita/h/home-2/mufma/Secondary/0.696halo0.dat","f","<"
+"f"*28,[505. * (1./2048.),7.9955,1.3313e10],[0,1,2,6,7,8,16],[0,0,0,1,1,1,2])

# you can provide directoires as follows: ['a1','a2',...]
# and corresponding data sets as follows: [['b11','b12',..],['b21','b22',...],...]
convert_array_to_h5py('Tryout.hdf5',['Positions','Velocities','Halo_Masses'],[['x','y','z'],['x','y','z'],['Halo_Masses']],info)
